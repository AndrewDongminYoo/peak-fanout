...
Every transaction that touches both a job and its reminder takes the job row first and the reminder row second — the completion, the dead-letter, and the failure's `FOR UPDATE` re-read all do — so two workers recording the same reclaimed job can wait on each other but never in a cycle, and Postgres never has to abort one of them as a deadlock.
