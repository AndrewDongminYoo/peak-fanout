...
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| loading     | spinner                                                                                                                                                                                                                                                                                                                     |
| loaded      | `timezone`, `reminder_time`, `push_token` (`null` renders as "not registered"), a "Register push notifications" button on iOS only while `push_token` is `null` (web never shows it: `expo-notifications` does not deliver Expo push tokens on web; Android never shows it until FCM is configured, below), sign-out button |
| registering | the loaded card with a spinner on the register button                                                                                                                                                                                                                                                                       |
...
Registration asks `expo-notifications` for permission, reads `Constants.expoConfig.extra.eas.projectId`, calls `getExpoPushTokenAsync({ projectId })`, and sends the token to `PUT /me/push-token` (`registerPushToken` in `src/lib/push-token.ts`).
