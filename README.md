...
Stop that scheduler before seeding again — its next tick would enqueue the fresh peak within the minute and the harness would refuse once more, and a tick that lands while the seed is deleting can deadlock against it, in which case a seed aborted that way rolls back and changes nothing — then seed, and start the scheduler with the mode.
