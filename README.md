...
The sign-out and account-switch clear sends the Expo push token the installation remembered at its last registration, so an account registered on two installations keeps the newer installation's token when the older one clears (issue #57, option 1).
Push tokens then moved to one `push_tokens` row per installation (issue #59, option 2 of #57): `PUT /me/push-token` adds or moves a row instead of replacing a column, `DELETE /me/push-token` deletes only the row the body names and nothing without a body, `GET /me` lists `push_tokens`, and both senders send once per token, one `deliveries` row per send.
A seeded user has no row, so the push distribution and every measured number are unchanged: `GET /me`, the API p95 instrument, reads the row and the list in the one statement it has always been, the enqueue tick reads ids without the token aggregate, and only the unmeasured write routes take a second read after their write.
The Me screen reconciles an installation that remembers nothing against the signed-in account's list, which covers its next sign-out but not an account switch that arrives before that card has loaded; the previous account's row for this installation then stays until the new account registers from it or the previous one signs in here again (design.md "Auth callback").
...
M5 stays open because [design.md](design.md) still assigns it `GET /admin/queue` for the demo dashboard and, in "The push sink", the full scheduled path for ordinary users and push-receipt polling; none of the three has a handler yet, and they are tracked as #61, #62 and #63.
