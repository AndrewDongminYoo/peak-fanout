...
The sign-out and account-switch clear sends the Expo push token the installation remembered at its last registration, so an account registered on two installations keeps the newer installation's token when the older one clears (issue #57, option 1); an installation with nothing remembered still clears unconditionally, and closing that shape needs the per-installation token rows of option 2, which are not planned here.
