...
The cache evidence is that replica counter change, not a faster fan-out: the simulated 50–150 ms push wait still dominates each batch, and 15.0 s is effectively the same duration as the 14.8 s M2 baseline on this one-machine run.
