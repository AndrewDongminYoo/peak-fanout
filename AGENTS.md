...
The repository is at the **end of M2 part 1** and is the Bun workspaces monorepo described in `README.md` under "Layout".
...
- `packages/db` (`@peak-fanout/db`) holds the Drizzle `users`, `reminders`, `jobs` and `deliveries` schema, `createDb(url)` over the `postgres` driver, five generated migrations under `drizzle/`, and the peak seed (`src/seed-plan.ts` decides the distribution, `src/seed.ts` writes it, `src/time.ts` converts local reminder times to UTC). `jobs` is the queue: `id, kind, payload jsonb, run_at, locked_at, locked_by, attempts, last_error, dead_at, done_at`, with a partial index on `(run_at) WHERE done_at IS NULL`; `reminders.state` walks `pending → queued → sent | failed`.
...
- `apps/api/src/load/` is the measured run (`bun run load:m1`), and `README.md`'s M1 row is filled from the run log it wrote. It does not yet know about workers; that is part 2.
...
M2 part 1 ends here: the `jobs` table, the enqueue tick, and a worker that claims with `SKIP LOCKED`, retries with backoff, dead-letters, is reclaimed by lease and shuts down gracefully — all of it tested without Postgres, and none of it measured.
M2 part 2 is next: the harness's support for N workers, the restart procedure behind the "jobs lost across worker restart" column, issues #25 and #26 with `RUN_LOG_SCHEMA_VERSION` 5, a re-measured M1 row and the first M2 row. Until it lands, the M2 row stays empty.
