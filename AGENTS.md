...
The repository is at the **end of M3 part 1** and is the Bun workspaces monorepo described in `README.md` under "Layout".
...
- `apps/api` (`@peak-fanout/api`) is an Elysia app with `GET /health`, `POST /auth/session`, `GET /me` and `GET /cards/today`. No authenticated route serves a row carrying `users.seeded`: `POST /auth/session` answers 409 and the two reads report it absent, because a seeded row is a load-test fixture rather than an identity. `src/app.ts` exports `createApp({ users, jwt, cards })` and `type App`; `src/index.ts` wires the read/write pair, the cards cache and the service, and listens only under `import.meta.main`, so tests build the app with an in-memory repository and a fake cards service and never open a port or a database connection.
...
- `apps/api/src/worker/` is the worker, N processes (`bun run dev:worker`, one per terminal) that claim `WORKER_BATCH_SIZE` jobs with `FOR UPDATE SKIP LOCKED`, read each reminder's cards for its local date through the cards module before the send, send them concurrently through the same sink, and record each outcome in its own transaction. A card read that throws skips that job — no send, nothing recorded, left claimed for the lease — as does a claimed job whose reminder no longer exists, and the batch line counts both as `skipped`; a batch skipped whole is followed by the poll sleep. A failed send is retried `WORKER_BACKOFF_BASE_MS × 2^(attempts − 1)` later and dead-lettered at `WORKER_MAX_ATTEMPTS`; a batch left locked by a killed worker is reclaimed after `WORKER_LEASE_MS`, which makes delivery at-least-once and is why every reminder update carries `WHERE state = 'queued'`. `SIGTERM` or `SIGINT` finishes the batch in flight and exits 0 at once, an idle worker included: the poll sleep is cut short and its timer released, so nothing holds the process open after the loop returns.
...
- `docker-compose.yml` runs a single `postgres-primary`. There is no read replica yet: `DATABASE_READ_URL` is read by the API and the worker and documented in `.env.example`, and M3 part 2 gives it a target.
...
M3 part 2 is next: the replica — `postgres-replica` in docker compose, `DATABASE_READ_URL` pointing at it, and `GET /deliveries?limit=` on `db.read`. Part 3 measures. Until it lands, the M3 row stays empty.
...
- `src/app.ts` — `createApp({ users, jwt, cards })` builds the Elysia app and `type App` is what the mobile app imports (`package.json` `exports` points here). Routes and their response schemas live here until a `src/routes/` split is worth it. Keep it free of Bun-only and database imports: `apps/mobile` typechecks this file, and `cards` is typed through `src/cards/service.ts`, which is kept free of them for the same reason.
...
- `src/cards/` — `pick.ts` is `positionsForDay(dayNumber, count)`, the arithmetic of design.md "The day's cards", pure; `cache.ts` is `createSwrCache` over a `Map` with an injected clock (fresh, stale with one revalidation behind it, expired; single-flight per key; LRU by `maxEntries`), the `CardsCache` interface a Redis implementation would also satisfy, `createPassThroughCache` for `CARDS_CACHE=off`, and `readCardsCacheConfig(env)` in the shape of `readWorkerConfig`; `cards-drizzle.ts` is `createDrizzleCardsRepository(db)` with `maxPosition()` and `byPositions(positions)`, the two statements, over `db.read`; `service.ts` is `createCardsService({ repository, cache })` with `forDate(localDate)` and `todayFor(instant, timezone)`, plus `messageFor(cards)`, the push copy (the M1 `REMINDER_MESSAGE` when there are no cards). `service.ts` imports `@peak-fanout/db/time`, the package's pure subpath, and never its root, because `app.ts` imports its type and `apps/mobile` typechecks `app.ts`; its value import of `REMINDER_MESSAGE` puts `scheduler/tick.ts` and `push/sink.ts` in that same typecheck program, so the rule that keeps Drizzle and Bun-only imports out of `tick.ts` now also protects the mobile build. The cache key is the local date alone. The SQL in `cards-drizzle.ts` is not unit-tested; validate it against the compose Postgres before a pull request opens.
...
- `src/seed-plan.ts` — the seed's numbers: the target instant, the timezone split, which local time each of the 50,000 indices gets, and `EXPRESSION_COUNT` (1,000) with `seedExpression(position)`, the content rule the expressions insert repeats in SQL. Change this, not the SQL, to change the distribution.
