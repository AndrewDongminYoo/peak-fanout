...
The repository is at the **end of M3 part 2** and is the Bun workspaces monorepo described in `README.md` under "Layout".
...
M3 part 2 ends here: the streaming `postgres-replica`, `DATABASE_READ_URL` pointing at it, and `GET /deliveries?limit=` reading the shared seeded sample on `db.read`. Part 3 measures the cache-off primary control, the cache-off replica control, the cache-on replica run and one restart run. Until those logs land, the M3 row stays empty.
