...
The repository has **M0 through M4 complete, with M5 in progress** and is the Bun workspaces monorepo described in `README.md` under "Layout".
...
- `apps/api/src/push/` is the push sink: one interface, the simulated implementation that owns every committed measurement, and the M5 Expo implementation. **The simulated distribution is the experiment**, so changing it invalidates every committed comparison. Workers default to it and use `expo-server-sdk` only under `PUSH_SINK=expo`; the naive scheduler and every load worker command stay simulated. `bun run push:expo` is the explicit one-message path for the later real-device check. An accepted Expo ticket is not device-delivery evidence.
...
bun run push:expo                # one external Expo push request; requires EXPO_PUSH_TOKEN and is not a local gate
