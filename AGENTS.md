...
- `packages/db` (`@peak-fanout/db`) holds the Drizzle `users`, `reminders` and `deliveries` schema, `createDb(url)` over the `postgres` driver, two generated migrations under `drizzle/`, and the peak seed (`src/seed-plan.ts` decides the distribution, `src/seed.ts` writes it, `src/time.ts` converts local reminder times to UTC).
...
- `src/seed.ts` — the seed script (`bun run db:seed`) and the materializer, which takes its population as a `users.email` pattern so the seed only writes reminders its own delete can remove. One statement per segment over `generate_series`; never a row-at-a-time loop.
